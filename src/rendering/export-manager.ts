...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">`;
  ...
  <metadata>
    <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
      ...
      <rdf:Description>
        <dc:title>${metadata.title || 'SIGL Scene'}</dc:title>
        <dc:creator>${metadata.author || 'SIGL Engine'}</dc:creator>
        <dc:date>${metadata.created.toISOString()}</dc:date>
        <dc:description>${metadata.description || 'Generated by SIGL Engine'}</dc:description>
      </rdf:Description>
    </rdf:RDF>
  </metadata>`;
      }
...
%%EOF`;
   ...
   * Validate export options
   */
  private validateExportOptions(options: ExportOptions): SIGLError[] {
  ...
  private generateMetadata(scene: SceneDefinition, dimensions: { width: number; height: number }): ExportMetadata {
    return {
      title: scene.name || 'SIGL Scene',
      description: scene.description || 'Scene generated by SIGL Engine',
      author: scene.metadata?.author || 'SIGL Engine',
      created: new Date(),
      software: 'SIGL Engine',
